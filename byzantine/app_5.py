...
def main(app_config=None):
    ...
    for other in range(6):
        if other != 5:
            socketlist.append(Socket("5", str(other), log_config=app_config.log_config)) 
    k=2 
    ...
    routine = Routine(socketlist,alice, eprlist,str(5))
    with alice:
        routine.start_routine()
    print("5's result is: ", routine.result)
...
class Routine:
    ...
    def subroutine_1(self):
        ...
        for socket in self._sockets:
            ...
            x+=int(other_bi)
        print(f"subroutine_1 for 5 is " + str(x))
    ...
    def subroutine_3(self):
        ...
        for socket in self._sockets:
            ...
            self.other_bi.append(other_bi)
            x+=int(other_bi)
        print(f"subroutine_3 for 5 is " + str(x))
        if x < (len(self.other_bi)+1)/3:
            self._bi=0
        elif x> (2*len(self.other_bi)+1)/3:
            return 1
    def start_routine(self):
        print(f"start_routine for 5 with bi= "+ str(self._bi))
        while True:
            self.subroutine_1()
    ...
    def quantumCoin(self):
        ...
        if int(self._name)==leader:
            ...
            for i,epr in enumerate(self._epr_socket): # comincio il teleport per mandare i qubit ai vari nodi
                ...
                m1=ghz[i].measure()
                print(f'5 m1 measured')
                
                m2=e.measure()
                print(f'5 m2 measured')
                self._conn.flush()
                print(f'5 flushed')
                print(f'5 m1 ' + str(m1))
                print(f'5 m2 ' + str(m2))
            ...
            print(f"5 wait for epr from leader")
            epr=self._epr_socket[leader].recv_keep()[0]
            print(f"5 received epr from leader")
            
            print(f"5 wait for corrections from leader")
            m1, m2 = self._sockets[leader].recv_structured().payload
            print(f"5 received corrections from leader")
            if m2 == 1:
                epr.X()
            if m1 == 1:
                epr.Z()

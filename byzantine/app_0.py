...
class Routine:
    ...
    def start_routine(self):
        print(f"start_routine for 0 with bi= "+ str(self._bi))
        while True:
            self.subroutine_1()
    ...
    def quantumCoin(self):
        ...
        if int(self._name)==leader:
            ...
            for i,epr in enumerate(self._epr_socket): # comincio il teleport per mandare i qubit ai vari nodi
                ...
                m1=ghz[i].measure()
                print(f'0 m1 measured')
                
                m2=e.measure()
                print(f'0 m2 measured')
                self._conn.flush()
                print(f'0 flushed')
                print(f'0 m1 ' + str(m1))
                print(f'0 m2 ' + str(m2))
            ...
            print(f"0 wait for epr from leader")
            epr=self._epr_socket[leader].recv_keep()[0]
            print(f"0 received epr from leader")
            
            print(f"0 wait for corrections from leader")
            m1, m2 = self._sockets[leader].recv_structured().payload
            print(f"0 received corrections from leader")
            if m2 == 1:
                epr.X()
            if m1 == 1:
                epr.Z()
